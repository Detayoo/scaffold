        ...
        {pl ? (
          <div className="space-y-4 pt-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium">{pl?.currency} {formatMoney(pl?.amountMinor ?? 0)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Status</p>
              <StatusBadge status={pl?.status ?? ""} size="sm" />
            </div>
            {pl?.channels && pl.channels.length > 0 && (
              <div>
                <p className="text-xs text-muted-foreground">Channels</p>
                <p className="text-sm capitalize">{pl.channels.join(", ")}</p>
              </div>
            )}
            {pl?.payUrl && (
              <div>
