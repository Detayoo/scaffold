              ...
              <InputOTPGroup className="flex w-full gap-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <InputOTPSlot
                    key={i}
                    index={i}
                    className="flex h-12 rounded-lg border border-input bg-background text-lg font-semibold text-foreground transition-all duration-150 data-[active=true]:border-foreground data-[active=true]:ring-2 data-[active=true]:ring-ring/50"
                  />
                ))}
              </InputOTPGroup>
            </InputOTP>
