...
import { RefundDetailSheet } from "@/modals/RefundDetailSheet";
import { getRefundsFn } from "@/services";
...
function RefundsContent() {
  ...
  const { data, isPending, isError, refetch, isFetching } = useQuery({
    queryKey: ["refunds", statusFilter, searchInput],
    queryFn: () => getRefundsFn({ reference: searchInput || undefined, status: statusFilter || undefined }),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        emptyDescription={searchInput ? "Try a different search term" : "No refund requests yet"}
        emptyAction={searchInput ? undefined : { label: "Create Refund", onClick: () => setCreateOpen(true) }}
        onRowClick={(r) => setDetailId(r?.id)}
      />

      <CreateRefundModal open={createOpen} onOpenChange={setCreateOpen} onSuccess={() => refetch()} />
      <RefundDetailSheet refundId={detailId} onOpenChange={(o) => { if (!o) setDetailId(null); }} />
    </div>
  );
}
